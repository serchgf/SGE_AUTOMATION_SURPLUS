...
from steps import MTC_SURSGE_023_steps as surp_steps
...
class MTC_SURSGE_023_Tests:
    ...
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_023_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_023(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar devolución total a permanente")
        print("------------------------------Test to Generar devolución total a permanente")
        ...
        surp_steps.MTC_SURSGE_023_steps(sge_funct, data, images.img_inicio_factura)
        sge_funct.segundos_de_espera(2)
        print("Validating Generar devolución total a permanente")
        if sge_funct.validate_image_x(images.img_validacion_devolucion):
            logging.info("Generar devolución total a permanente success")
            print("Generar devolución total a permanente success")
        else:
