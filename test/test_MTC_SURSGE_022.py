...
from steps import MTC_SURSGE_022_steps as surp_steps
...
# @ddt
class MTC_SURSGE_022_Tests:

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_022_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_022(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar Proceso de localizaciones")
        print("------------------------------Test to Generar Proceso de localizaciones")
        ...
        surp_steps.MTC_SURSGE_022_steps(sge_funct, data)
        sge_funct.segundos_de_espera(2)
        print("Validating Generar Proceso de localizaciones")
        if sge_funct.validate_image_x(images.img_validacion_tc22):
            logging.info("Generar Proceso de localizaciones success")
            print("Generar Proceso de localizaciones success")
            message = "Proceso_de_localizaciones_successfull"
            sge_funct.take_screenshot(message)
            sge_funct.terminate_sge_session()
        else:
            logging.error("Error, validation Failed")
            print("Error, validation Failed")
            message = "fallo_la_validacion"
            sge_funct.take_screenshot(message)
            sge_funct.terminate_sge_session()
            assert False
