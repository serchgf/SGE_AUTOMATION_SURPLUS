...
from steps import MTC_SURSGE_024_steps as surp_steps
...
# @ddt
class MTC_SURSGE_023_Tests:

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_024_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_024(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar devolución parcial a permanente")
        print("------------------------------Test to Generar devolución parcial a permanente")
        ...
        surp_steps.MTC_SURSGE_024_steps(sge_funct, data, images.img_inicio_factura)
        sge_funct.segundos_de_espera(2)
        print("Validating Generar devolución parcial a permanente")
        if sge_funct.validate_image_x(images.img_validacion_devolucion):
            logging.info("G.enerar devolución parcial a permanente success")
            print("Generar devolución parcial a permanente success")
        else:
