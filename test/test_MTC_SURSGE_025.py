...
from steps import MTC_SURSGE_025_steps as surp_steps
...
# @ddt
class MTC_SURSGE_025_Tests:

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_025_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_025(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar devolución total a multiple")
        print("------------------------------Test to Generar devolución total a multiple")
        ...
        surp_steps.MTC_SURSGE_025_steps(sge_funct, data, images.img_inicio_factura)
        sge_funct.segundos_de_espera(2)
        print("Validating Generar devolución total a multiple")
        if sge_funct.validate_image_x(images.img_validacion_devolucion):
            logging.info("Generar devolución total a multiple success")
            print("Generar devolución total a multiple success")
        else:
