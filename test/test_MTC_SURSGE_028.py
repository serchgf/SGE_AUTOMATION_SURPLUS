...
from steps import MTC_SURSGE_028_steps as surp_steps
...
# @ddt
class MTC_SURSGE_028_Tests:

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_028_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_028(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar traspaso SUC a CEDI normal parcial masiva a permanente")
        print("------------------------------Test to Generar traspaso SUC a CEDI normal parcial masiva a permanente")
        ...
        surp_steps.MTC_SURSGE_028_steps(sge_funct, data, images.img_vale_interno)
        print("Validating Generar traspaso SUC a CEDI normal parcial masiva a permanente")
        if sge_funct.validate_image_x(images.img_validacion_traspaso):
            logging.info("Generar traspaso SUC a CEDI normal parcial masiva a permanente success")
            print("Generar traspaso SUC a CEDI normal parcial masiva a permanente success")
        else:
