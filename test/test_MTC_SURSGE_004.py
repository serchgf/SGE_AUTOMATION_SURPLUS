...
from steps import MTC_SURSGE_004_steps as sur_steps
# from ddt import ddt, file_data
from Config.config import images


# @ddt
class MTC_SURSGE_004_Tests(unittest.TestCase):

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_004(self):
        sge_funct = sge_functions2()
        logging.info("------------Test to Create_zonas_carga_masiva------------------")
        print("------------Test to Create_zonas_carga_masiva------------------")
        ...
        sur_steps.MTC_SURSGE_004_steps(sge_funct, txt_files_path.cm_zonas_txt)
        print("Validating carga masiva Zonas")
        assert sge_funct.validate_image_x(images.img_zonas_cm_guardadas), "Error, Zonas Carga masiva not created"
        print("Zonas carga masiva created successfull")
