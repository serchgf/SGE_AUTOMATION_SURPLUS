...
from steps import MTC_SURSGE_027_steps as surp_steps
...
# @ddt
class MTC_SURSGE_027_Tests:

    # @file_data("./Config/TestDataset/dataSet_ddt_invoice_credit_simple_order.json")
    @pytest.mark.parametrize("data", cargar_datos_prueba(files.mtc_sursge_027_json))
    # @pytest.mark.flaky(reruns=1)
    def test_MTC_SURSGE_027(self, data):
        # print("Test using: "+supplier)

        sge_funct = sge_functions2()
        logging.info("------------------------------Test to Generar traspaso CEDI a CEDI normal total manual a permanente")
        print("------------------------------Test to Generar traspaso CEDI a CEDI normal total manual a permanente")
        ...
        surp_steps.MTC_SURSGE_027_steps(sge_funct,  data, images.img_vale_interno, images.img_folioEmbarque, files.cm_traspasos_file,)
        sge_funct.segundos_de_espera(2)
        print("Validating Generar traspaso CEDI a CEDI normal total manual a permanente")
        if sge_funct.validate_image_x(images.img_validacion_traspaso):
            logging.info("Generar traspaso CEDI a CEDI normal total manual a permanente success")
            print("Generar traspaso CEDI a CEDI normal total manual a permanente success")
        else:
