...
class sge_functions2:
    ...
    def validate_image_x(self, img_to_validate) -> bool:
        logging.info("Validate image presence on screen")
        time.sleep(2)
        in_screen = pa.locateOnScreen(img_to_validate)
        logging.info(f"{in_screen}")

        if in_screen is None or in_screen == "":
            logging.info(f"{in_screen}")
            #print(in_screen)
            message = "Error_not_found_on_screen"
            self.take_screenshot(message)
            return False
    ...
    def escribir_wz_temporal(self):
        pa.typewrite("T_WZ_01P", interval=0.1)

    def escribir_intervalo(self, text):
    ...
    def doubleclick_axis_y(self, img_reference, add_pixels_on_axis_y: int):
        ...
        """
        :param img_reference:
        :param add_pixels_on_axis_y: pixels to add or reduce from img_reference
        :return: double click action on new axis y coordinates y regresa el valor del texto seleccionado si existe
        """
        img_coordinates = pa.locateCenterOnScreen(img_reference)
        #print("coordenadas de double click")
        #print(img_coordinates)
